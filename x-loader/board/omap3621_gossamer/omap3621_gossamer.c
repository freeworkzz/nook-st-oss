...
#define MUX_DEFAULT()\
	...
	MUX_VAL(CP(DSS_DATA20),     (IEN  | PTD | DIS | M4)) /*GIO-90*/\
...
inline void gpio_led(void)
{
volatile unsigned long buff;
#define GOSSAMER_DEBUG_LED_GPIO   90
#define GPIO3_BASE    		  0x49052000
#define GPIO3_OE              	  GPIO3_BASE+0x34
#define GPIO3_DATAOUT         	  GPIO3_BASE+0x3C

	buff = *(volatile unsigned long*)CM_FCLKEN_PER;		// CM_FCLKEN_PER
	buff |= (1<<14);
	__raw_writew(buff, CM_FCLKEN_PER);
	buff = *(volatile unsigned long*)CM_ICLKEN_PER;		// CM_ICLKEN_PER
	buff |= (1<<14);
	__raw_writew(buff, CM_ICLKEN_PER);

	buff = *(volatile unsigned long*)GPIO3_OE;		// gpio 90 OE reg
	buff &= (~(1<<(GOSSAMER_DEBUG_LED_GPIO-64)));
	__raw_writel(buff, GPIO3_OE);				// gpio 90

	buff = *(volatile unsigned long*)GPIO3_DATAOUT;		// read output register
	buff &= (~(1<<(GOSSAMER_DEBUG_LED_GPIO-64)));		// clear debug led
	__raw_writel(buff, GPIO3_DATAOUT);			// set
}
