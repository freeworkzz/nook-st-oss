...
void HTMLAnchorElement::defaultEventHandler(Event* evt)
{
	...
	const String gmailurl = "http://m.gmail.com" ;
	String url ;
	String tempurl =  getAttribute(hrefAttr);

	if ( tempurl == "http://mail.google.com/mail/?hl=en&tab=wm" )
	  url =  deprecatedParseURL(gmailurl);
	else
          url = deprecatedParseURL(tempurl);
